...
import argparse
import requests
from instruments import Secchi
...
def main(server=False, logs=False, remove_api_data=False):
    ...
    if logs:
        log = logger(os.path.join(repo, "logs/secchidepth"))
    else:
        log = logger()
    log.initialise("Processing LéXPLORE secchi depth data")
    ...
    if server:
        ...
        new_files = retrieve_new_files(directories["failed"],
                                       creds, server_location="data/Secchi",
    ...
    for file in files:
        if ".csv" in file:
            sensor = Secchi(log=log)
            if sensor.read_data(file):
                sensor.quality_assurance(file_path="notes/quality_assurance.json")
                edited_files.extend(sensor.export(directories["Level1"], "L1_Secchi_Depth", output_period="file"))
    log.end_stage()
